...
LFR = zeros(size(LF));
GFR = zeros(size(GF));
fgf = fit(time',GF,'smoothingspline');
flf = fit(time',LF,'smoothingspline');
...
GFR = smooth(GFR);
LFR = smooth(LFR);
[GFL_peaks,GFL_T] = findpeaks(GF,'MinPeakHeight',5,'NPeaks',10,'MinPeakDistance',50);
...
for l = 1:leng
  if(GFL_T(l)-num~= 0 && GFL_T(l)+num<=length(time))
        tnew = time(GFL_T(l)-num):1e-3:time(GFL_T(l));
        pos2 = find(fgf(tnew)>=0.1 & fgf(tnew)<0.11);                                       %Determines >=0.1N GF
        pos1 = pos2(1)+find(flf(tnew(pos2(1):end))>=0.1 & flf(tnew(pos2(1):end))<0.11);     %Determines >=0.1N LF
        LFRsub = LFR(GFL_T(l)-num:GFL_T(l));
        GFRsub = GFR(GFL_T(l)-num:GFL_T(l));
        [LFR_peaks(l),LFR_T(l)] = findpeaks(LFRsub,'MinPeakHeight',11,'NPeaks',1);
        LFR_T(l) = GFL_T(l)-num + LFR_T(l);
        [GFR_peaks(l),GFR_T(l)] = findpeaks(GFRsub,'MinPeakHeight',11,'NPeaks',1);
        GFR_T(l) = GFL_T(l)-num + GFR_T(l);
        if length(pos1)>0
            PLD(l) = (tnew(pos1(1))-tnew(pos2(1)))*1000;
            [val,idx] = min(abs(time(GFL_T(l)-num:GFL_T(l))-tnew(pos1(1))));
            GFO(l) = rGF(GFL_T(l)-num+idx+1);
        else
            PLD(l) = NaN;
            GFO(l) = NaN;
        end
...
fprintf( fid1, '%s,%s,%s,%s,%s,%.2d,%.2d,%.2d,%.2d,%.2d',char(St(1)),char(St(3)),char(St(4)),char(St(2)),strcat('Trial--', int2str(k)),LFR_peaks(k),GFR_peaks(k),PLD(k),GFO(k),GFL_peaks(k));
fprintf( fid1,'\n');
end
